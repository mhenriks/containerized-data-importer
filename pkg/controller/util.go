...
func CreateUploadPod(client kubernetes.Interface, caKeyPair *triple.KeyPair, image, verbose, pullPolicy, name string, pvc *v1.PersistentVolumeClaim) (*v1.Pod, error) {
	...
	glog.V(Vuser).Infof("upload pod \"%s/%s\" (image: %q) created\n", pod.Namespace, pod.Name, image)
	return pod, nil
}
...
func MakeUploadPodSpec(image, verbose, pullPolicy, name string, pvc *v1.PersistentVolumeClaim, secretName string) *v1.Pod {
	...
	pod := &v1.Pod{
		...
		ObjectMeta: metav1.ObjectMeta{
			...
			Labels: map[string]string{
				CDI_LABEL_KEY:               CDI_LABEL_VALUE,
				UPLOAD_SERVER_SERVICE_LABEL: name,
			},
			OwnerReferences: []metav1.OwnerReference{
		...
		Spec: v1.PodSpec{
			Containers: []v1.Container{
				{
					Name:            UPLOAD_SERVER_PODNAME,
					Image:           image,
					ImagePullPolicy: v1.PullPolicy(pullPolicy),
					VolumeMounts: []v1.VolumeMount{
						{
							Name:      DataVolName,
							MountPath: UPLOAD_SERVER_DATA_DIR,
						},
						{
							Name:      secretName,
...
func CreateUploadService(client kubernetes.Interface, name string, pvc *v1.PersistentVolumeClaim) (*v1.Service, error) {
	...
	glog.V(Vuser).Infof("upload service \"%s/%s\" created\n", service.Namespace, service.Name)
	return service, nil
}
